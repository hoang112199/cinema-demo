...
function filledMovieForm() {
	...
	if ($('#datepicker-manager-3')[0].value == '' || $('#datepicker-manager-4')[0].value == '' ||
		...
		$('#manager-dynamic-3').html('<h5>Điền mọi thông tin</h5>');
	else if (!(atleastTypes.includes(types[0]) && allTypes.includes(types[1]) && allTypes.includes(types[2])))
		$('#manager-dynamic-3').html('<h5>Loại phim không hợp lệ</h5>');
	else if (!$.isNumeric(movieLen))
		$('#manager-dynamic-3').html('<h5>Chiều dài phim phải là 1 con só</h5>');
	else if (Date.parse(startShowing) > Date.parse(endShowing))
		$('#manager-dynamic-3').html("<h5>Ngày công chiếu phải trước ngày kết thúc</h5>");
	else {
...
function updateMovieFunction(_movieID) {
	...
	$('#manager-dynamic-3').html('<input type="text" name="newMovieName" placeholder="Tên Phim ">');
	$('#manager-dynamic-4').html('<input type="text" name="newMovieLen" placeholder="Chiều Dài Phim">');
	$('#manager-dynamic-5').html('<input type="text" name="newMovieLanguage" placeholder="Ngôn Ngữ>');
	$('#manager-dynamic-6').html('<input type="text" name="newMovieTypes" placeholder="Loại Phim">');
	$('#manager-dynamic-7').html('<input id="datepicker-manager-7" placeholder="Ngày Bắt Đầu">');
	...
	$('#manager-dynamic-8').html('<input id="datepicker-manager-8" placeholder="Ngày Kết Thúc">');
	...
	$('#manager-dynamic-9').html('<button onclick="changeMovieInfo()">Thay đổi</button>')
}
...
function changeMovieInfo() {
	...
	if ($('#datepicker-manager-7')[0].value == '' || $('#datepicker-manager-8')[0].value == '' ||
		...
		$('#manager-dynamic-10').html('<h5>Điền mọi thông tin</h5>');
	else if (!(atleastTypes.includes(types[0]) && allTypes.includes(types[1]) && allTypes.includes(types[2])))
		$('#manager-dynamic-10').html('<h5>Loại phim không hợp lệ</h5>');
	else if (!$.isNumeric(newMovieLen))
		$('#manager-dynamic-10').html('<h5>Độ dài phim phải là một con số</h5>');
	else if (Date.parse(startShowing) > Date.parse(endShowing))
		$('#manager-dynamic-10').html("<h5>Ngày khởi chiếu phải trước ngày kết thúc</h5>");
	else {
...
function deleteMovieFunction(_movieID) {
	movieID = _movieID;
	$('#manager-dynamic-2 button').prop('disabled', true);
	$('#manager-dynamic-3').html('<button onclick="deleteMovieInfo()">Xóa</button>')
...
function SearchMovie() {
	$('#options button').prop('disabled', true);
	$('#manager-dynamic-3').html('<input type="text" name="SearchMovieName" placeholder="Tìm kiếm bằng tên phim"><button onclick="searchMovieInfo()">Tìm kiếm</button>');
...
function createShow() {
	$('#options button').prop('disabled', true);
	$('#manager-dynamic-2').html('<input id="datepicker-manager-4" placeholder="Chọn một ngày"><input id="timepicker-manager-2" placeholder="Chọn 1 khoảng thời gian"><button onclick="getValidMovies()">Xác nhận</button>');
	$('#datepicker-manager-4').pickadate({
		formatSubmit: 'yyyy/mm/dd',
		hiddenName: true,
		min: new Date(),
	...
	$('#timepicker-manager-2').pickatime({
		formatSubmit: 'HHi',
		hiddenName: true,
		interval: 15,
...
function getValidMovies() {
	if ($('#timepicker-manager-2')[0].value == '' || $('#datepicker-manager-4')[0].value == '') {
		$('#manager-dynamic-3').html('<h5>Điền vào tất cả các trường</h5>');
		return;
	}
...
function selectShowMovie(movID, types) {
	...
	$('#manager-dynamic-4').html('<h4>Chọn Loại Phim</h4>');
	types.split(' ').forEach(function (t) {
		$('#manager-dynamic-4').append('<button onclick="selectShowType(' + ("'" + t + "'") + ')">' + t + '</button>');
	});
}
...
function selectShowHall(hall) {
	$.ajax({
		...
		success: function (response) {
			$('#manager-dynamic-5 button').prop('disabled', true);
			$('#manager-dynamic-6').html(response);
		}
	});
}
...
function alterPrice(mpriceID) {
	...
	$('#manager-dynamic-2').html('<input type="number" name="new_price" placeholder="Giá mới cho loại ghế thường VND"><button onclick="changePrice()">Thay đổi</button>');
}
...
function filledStaffForm() {
	...
	if (staffName == '' || staffIDcard == '' || staffGender == ''|| staffPhoneNumber == ''|| staffEmail == ''|| staffAddress == '' || staffPosition == '' || staffSalary == ''|| $('#datepicker-manager-3')[0].value == '') {
		$('#manager-dynamic-4').html('<h5>Điền mọi thông tin</h5>');
	}
	else if (!$.isNumeric(staffIDcard)) {
		$('#manager-dynamic-4').html('<h5>CMND phải là một dãy số</h5>');
	}
	else if (!$.isNumeric(staffPhoneNumber)) {
		$('#manager-dynamic-4').html('<h5>Số điện thoại phải là một dãy số</h5>');
	}
	else {
...
function updateMemberFunction(_memberID) {
	memberID = _memberID;
	$('#manager-dynamic-2 button').prop('disabled', true);
	$('#manager-dynamic-3').html('<input type="text" name="newMemberName" placeholder="Tên khách hàng">');
	$('#manager-dynamic-4').html('<input id="datepicker-manager-4" name="newMemberDob" placeholder="Ngày sinh">');
	$('#datepicker-manager-4').pickadate({
		formatSubmit: 'yyyy/mm/dd',
		hiddenName: true,
		onSet: function (event) {
	...
	$('#manager-dynamic-5').html('<input type="text" name="newMemberGender" placeholder="Giới tính">');
	$('#manager-dynamic-6').html('<input type="text" name="newMemberIDcard" placeholder="CMND">');
	$('#manager-dynamic-7').html('<input type="text" name="newMemberPhoneNumber" placeholder="Số điện thoại">');
	$('#manager-dynamic-8').html('<input type="text" name="newMemberEmail" placeholder="Email">');
	$('#manager-dynamic-9').html('<input type="text" name="newMemberType" placeholder="New Type"><button onclick="changeMemberInfo()">Thay đổi</button>');
}
...
function deleteMemberFunction(_memberID) {
	memberID = _memberID;
	$('#manager-dynamic-2 button').prop('disabled', true);
	$('#manager-dynamic-3').html('<button onclick="deleteMemberInfo()">Xóa</button>')
...
function SearchMember() {
	$('#options button').prop('disabled', true);
	$('#manager-dynamic-3').html('<input type="text" name="SearchMemberName" placeholder="Tìm kiếm theo tên"><button onclick="searchMemberInfo()">Search</button>');
